...
def find_material(export_settings, material):
    ...
    project_dir = export_settings["project_path_func"]()
    mat_path = material.name.replace("res:/", project_dir)
    if os.path.exists(mat_path):
        with open(mat_path) as mat_file:
            first_line = mat_file.readline()
            if "SpatialMaterial" in first_line:
                return (mat_path, "SpatialMaterial")
            if "ShaderMaterial" in first_line:
                return (mat_path, "ShaderMaterial")
    raise Exception("Material not found at {}".format(mat_path))
